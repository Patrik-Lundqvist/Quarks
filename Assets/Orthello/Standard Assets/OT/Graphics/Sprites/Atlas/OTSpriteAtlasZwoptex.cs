						...
						if (!dsTextureNames.EOF && !dsTextures.EOF && dsTextureNames.rowCount == dsTextures.rowCount)
						{
			                try
			                {
								...
								{														
				                    ...
				                    var rotated = GetBool(dsTextures,"textureRotated");
				                    var frame = GetRect(dsTextures,"textureRect");
				                    var colorRect = GetRect(dsTextures, "spriteColorRect");
				                    var sourceSize = GetVector2(dsTextures, "spriteSourceSize");									
			                        try
			                        {
